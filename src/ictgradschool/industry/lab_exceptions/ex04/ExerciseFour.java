...
public class ExerciseFour {
    ...
    public void start() {
        ...
        try {
            ...
            e1.getMessage();
        } catch (IndexTooLowException e2) {
            e2.getMessage();
        } catch (IndexTooHighException e3) {
            e3.getMessage();
        }
    ...
    private int getArrayIndexFromUser() throws InvalidIndexException, IndexTooLowException, IndexTooHighException {
        ...
        String enter = Keyboard.readInput();
        if (!(enter == null)) {
            throw new InvalidIndexException("doesn’t enter an integer");
        }
        int index = Integer.parseInt(enter);
        if (index < 0) {
            throw new IndexTooLowException("enters a number that’s too small");
        }
        if (index > 4) {
            throw new IndexTooHighException("enters a number that’s too large");
        }
        return index;
